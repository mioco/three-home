...
function navFade() {
  var type = onTheGround ? 'add' : 'remove'
  Array.from(document.querySelector('nav ul').children).forEach((item, i) => setTimeout(() => item.classList[type]('hidden'), 100 * i))
}
...
if (!mobilecheck()) {
  ...
  motion.position.set(0, -55, 70);

  // game systems code

  var resetPlayer = function() {
    if( motion.position.y < -123 ) {
      motion.position.set(0, -55, 00);
  ...
  var keyboardControls = (function() {
    ...
    return function() {
      if( !motion.airborne ) {
        ...
        forward.multiplyScalar( keysPressed[keys.W] ? -1 : (keysPressed[keys.S] ? 1: 0));
        sideways.multiplyScalar( keysPressed[keys.A] ? -1 : (keysPressed[keys.D] ? 1 : 0));
        ...
        //jump
        var vy = keysPressed[keys.SP] && onTheGround ? 1 : 0;
      ...
      if( !motion.airborne ) {
        for( var j = 0, n = pads.length; j < n; j++ ) {
          if ( pads[j].distanceToSquared( motion.position ) < 2.3 ) {
            ...
            temp.copy( pads[j] ); temp.y = 0; temp.setLength( -0.8 ); temp.y = 1;
